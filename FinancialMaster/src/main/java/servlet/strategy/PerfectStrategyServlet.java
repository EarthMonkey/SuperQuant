...
public class PerfectStrategyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < stockId.length; i++) {
			ArrayList<profitPO> result = psService.getProfit(stockId[i], startTime[i], endTime[i],
					Double.parseDouble(cost[i]));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for (UserStrategy userStrategy2 : strategies) {
			...
			perST += userStrategyId.getStockId() + "," + userStrategy2.getWeight() + "," + userStrategy2.getStarttime()
					+ "," + userStrategy2.getEndtime() + "," + userStrategy2.getFrequency() + ";";
		...
		ArrayList<profitPO> profitPOs = strategyHandleService.handle(arrayList_buy, arrayList_sold);
		int length = profitPOs.size();
		profitPO po = profitPOs.get(length - 1);

		String data = "[{'profit':" + po.getProfit() + "}]";
